...
class FinancialDashboard:
    ...
    def load_csv_data(self, uploaded_file):
        ...
        try:
            ...
            if missing_columns:
                ...
                st.info(
                    "Required columns: customer_id, customer_name, monthly_income, monthly_expenses, savings_balance, investment_balance, total_debt, payment_history_score, credit_utilization_ratio, credit_age_months"
                )
                return None
    ...
    def display_customer_overview(self):
        ...
        avg_credit_score = (
            sum(r["credit_score"] for r in self.analysis_results) / total_customers
        )
        avg_risk_score = (
            sum(r["risk_assessment"]["risk_score"] for r in self.analysis_results)
            / total_customers
        )
        approved_count = sum(
            1
            for r in self.analysis_results
            if r["lending_recommendations"]["loan_approval"]
        )
        ...
        with col1:
            ...
            risk_df = pd.DataFrame(
                list(risk_counts.items()), columns=["Risk Level", "Count"]
            )
            fig = px.pie(
                risk_df,
                values="Count",
        ...
        with col2:
            ...
            health_df = pd.DataFrame(
                list(health_counts.items()), columns=["Health Category", "Count"]
            )
            fig = px.bar(
                health_df,
                x="Health Category",
        ...
        with col3:
            ...
            credit_df = pd.DataFrame(
                list(credit_ranges.items()), columns=["Credit Range", "Count"]
            )
            fig = px.bar(
                credit_df,
                x="Credit Range",
        ...
        if avg_credit_score < 600:
            st.warning(
                "Average credit score is low. Consider focusing on credit-building products or stricter lending criteria."
            )
        elif avg_credit_score >= 700:
            st.success(
                "Average credit score is healthy. Portfolio is well-positioned for lending."
            )
        else:
        ...
        if avg_risk_score > 4:
            ...
            st.success(
                "Portfolio has low risk profile. Consider expanding lending criteria."
            )
        else:
        ...
        if approval_rate < 30:
            st.warning(
                "Approval rate is low. Review approval criteria or target segments."
            )
        ...
        if hasattr(self, "ml_df") and self.ml_df is not None:
            ...
            if label_col is None:
                st.error(
                    "No label column (Predicted_Label or Label) found in uploaded data!"
                )
                return
            # Ensure Amount is numeric
            self.ml_df["Amount"] = pd.to_numeric(self.ml_df["Amount"], errors="coerce")
            # Normalize label values
            self.ml_df[label_col] = (
                self.ml_df[label_col].astype(str).str.strip().str.upper()
            )
            st.write(
                f"[DEBUG] Unique values in Type column: {self.ml_df['Type'].unique()}"
            )
            st.write(f"[DEBUG] Label column: {label_col}")
            st.write(
                f"[DEBUG] Label value counts: {self.ml_df[label_col].value_counts().to_dict()}"
            )
            ...
            total_income = self.ml_df.loc[
                self.ml_df[label_col] == "INCOME", "Amount"
            ].sum()
            total_expenses = self.ml_df.loc[
                self.ml_df[label_col] == "EXPENSE", "Amount"
            ].sum()
            total_savings = self.ml_df.loc[
                self.ml_df[label_col] == "SAVINGS", "Amount"
            ].sum()
            ...
            total_credit = self.ml_df.loc[
                self.ml_df["Type"] == "CREDIT", "Amount"
            ].sum()
        ...
        if hasattr(self, "ml_df") and self.ml_df is not None:
            ...
            st.dataframe(
                self.ml_df[["Date", "Description", "Amount", "Type", "Predicted_Label"]]
            )
            # Export button for manual labeling
            csv_export = self.ml_df[
                ["Date", "Description", "Amount", "Type", "Predicted_Label"]
            ].to_csv(index=False)
            st.download_button(
                "Export Transactions for Labeling",
                data=csv_export,
                file_name="transactions_for_labeling.csv",
                mime="text/csv",
            )
    ...
    def display_credit_score_analysis(self):
        ...
        if poor / len(credit_scores) > 0.5:
            st.error(
                "Majority of customers have poor credit. Tighten lending or offer credit improvement programs."
            )
        elif good + very_good + exceptional > len(credit_scores) * 0.5:
            st.success(
                "Most customers have good or better credit. Consider pre-approved offers."
            )
        else:
    ...
    def display_financial_health_analysis(self):
        ...
        health_scores = [
            r["financial_health"]["financial_health_score"]
            for r in self.analysis_results
        ]
        health_categories = [
            r["financial_health"]["health_category"] for r in self.analysis_results
        ]
        dti_ratios = [
            r["financial_health"]["debt_to_income_ratio"] for r in self.analysis_results
        ]
        savings_rates = [
            r["financial_health"]["savings_rate"] for r in self.analysis_results
        ]
        ...
        with col1:
            ...
            fig.add_vline(
                x=20, line_dash="dash", line_color="red", annotation_text="Critical"
            )
            fig.add_vline(
                x=40, line_dash="dash", line_color="orange", annotation_text="Poor"
            )
            fig.add_vline(
                x=60, line_dash="dash", line_color="yellow", annotation_text="Fair"
            )
            fig.add_vline(
                x=80, line_dash="dash", line_color="green", annotation_text="Excellent"
            )
            st.plotly_chart(fig, use_container_width=True)
        ...
        if low_health / len(health_scores) > 0.5:
            st.error(
                "Most customers have poor financial health. Consider offering financial wellness programs."
            )
    ...
    def display_lending_recommendations(self):
        ...
        rejected_customers = [
            r
            for r in self.analysis_results
            if not r["lending_recommendations"]["loan_approval"]
        ]
                    ...
                    [
                        {
                            ...
                            "Interest Rate": r["lending_recommendations"][
                                "interest_rate_range"
                            ],
                        }
    ...
    def display_individual_customer_analysis(self):
        ...
        # Customer selector
        customer_options = [
            f"{r['customer_name']} ({r['customer_id']})" for r in self.analysis_results
        ]
        ...
        if selected_customer:
            ...
            customer_result = next(
                r for r in self.analysis_results if r["customer_id"] == customer_id
            )
            ...
            with col1:
                ...
                st.write(
                    f"**Credit Score:** {customer_result['credit_score']} ({customer_result['credit_rating']})"
                )
                st.write(
                    f"**Risk Level:** {customer_result['risk_assessment']['risk_level']}"
                )
                st.write(
                    f"**Financial Health:** {customer_result['financial_health']['health_category']}"
                )
                ...
                st.write(
                    f"**Investment Balance:** ₹{metrics['investment_balance']:,.0f}"
                )
            ...
            with col2:
                ...
                st.write(
                    f"**Emergency Fund Ratio:** {health['emergency_fund_ratio']:.1f} months"
                )
                st.write(f"**Savings Rate:** {health['savings_rate']:.1%}")
                st.write(
                    f"**Debt-to-Income Ratio:** {health['debt_to_income_ratio']:.1%}"
                )
                ...
                if lending["loan_approval"]:
                    st.success("✅ **APPROVED**")
                    st.write(
                        f"**Recommended Amount:** ₹{lending['recommended_loan_amount']:,.0f}"
                    )
                    st.write(
                        f"**Interest Rate Range:** {lending['interest_rate_range']}"
                    )
                else:
                    st.error("❌ **REJECTED**")
            ...
            st.subheader("Insights & Suggestions")
            if (
                customer_result["credit_score"] >= 740
                and customer_result["risk_assessment"]["risk_level"] == "Low"
            ):
                st.success(
                    "Excellent candidate for premium products or pre-approved loans."
                )
            elif customer_result["risk_assessment"]["risk_level"] in [
                "High",
                "Very High",
            ]:
                st.error(
                    "High risk customer. Recommend additional due diligence or risk mitigation."
                )
            elif customer_result["financial_health"]["health_category"] in [
                "Poor",
                "Critical",
            ]:
                st.warning(
                    "Customer has poor financial health. Suggest financial counseling or restricted lending."
                )
            else:
                st.info(
                    "Customer is in a moderate segment. Review full profile before proceeding."
                )
    ...
    def display_insights_and_segmentation(self):
        ...
        with col3:
            st.metric(
                "Net Cash Flow",
                f"₹{net_cash_flow:,.0f}",
                delta=f"{savings_rate:.1%} Savings Rate",
            )
        ...
        if "analysis_date" in customer:
            st.info(
                "Trends are based on the uploaded statement period. For multi-month trends, upload a longer statement."
            )
        ...
        if income_segment == "High Income" and savings_segment == "Saver":
            st.success(
                "This customer is financially strong and a low-risk candidate for most banking products."
            )
        elif savings_segment == "High Spender":
            st.warning(
                "Customer spends most of their income. Consider counseling on savings or offering budgeting tools."
            )
    ...
    def display_ml_prediction_tab(self):
        ...
        if not all(key in first_result for key in min_required_keys):
            st.info(
                "Loan approval prediction is only available for customer-level or auto-generated profiles. Please upload a customer data CSV file or a bank statement."
            )
            return
        # For auto-generated profiles, fill in missing advanced fields with defaults
        customer_options = [
            f"Customer {i+1}" for i in range(len(self.analysis_results))
        ]
        selected_customer = st.selectbox(
            "Select Customer for ML Prediction", customer_options
        )
        if selected_customer:
            ...
            if ml_pred is not None:
                    ...
                    go.Indicator(
                        ...
                        gauge={
                            ...
                            "steps": [
                                {
                                    "range": [0, 50],
                                    "color": "#ffe6e6" if ml_pred == 0 else "#e6ffe6",
                                },
                                {
                                    "range": [50, 100],
                                    "color": "#e6ffe6" if ml_pred == 1 else "#ffe6e6",
                                },
                            ],
                        },
                        number={"suffix": "%"},
                ...
                if ml_pred == 1 and ml_prob > 0.8:
                    st.success(
                        "This customer is a strong candidate for loan approval. Consider offering premium products or pre-approved offers."
                    )
                elif ml_pred == 1:
                    st.info(
                        "Customer is likely to be approved, but review full profile for risk factors."
                    )
                elif ml_pred == 0 and ml_prob > 0.8:
                    st.error(
                        "Customer is a high risk for loan rejection. Consider additional documentation or risk mitigation."
                    )
                else:
                    st.warning(
                        "Customer is borderline for approval. Manual review recommended."
                    )
            else:
    ...
    def export_results(self):
        ...
        with pd.ExcelWriter(
            "financial_analysis_report.xlsx", engine="xlsxwriter"
        ) as writer:
                ...
                summary_data.append(
                    {
                        ...
                        "Financial Health Score": r["financial_health"][
                            "financial_health_score"
                        ],
                        "Health Category": r["financial_health"]["health_category"],
                        "Loan Approval": r["lending_recommendations"]["loan_approval"],
                        "Recommended Amount": r["lending_recommendations"][
                            "recommended_loan_amount"
                        ],
                        "Interest Rate Range": r["lending_recommendations"][
                            "interest_rate_range"
                        ],
                        ...
                        "Emergency Fund Ratio": r["financial_health"][
                ...
                lending_data.append(
                    {
                        ...
                        "Loan Terms": ", ".join(
                            r["lending_recommendations"]["loan_terms"]
                        ),
                        "Conditions": ", ".join(
                            r["lending_recommendations"]["conditions"]
                        ),
                        "Risk Mitigation": ", ".join(
                            r["lending_recommendations"]["risk_mitigation"]
                        ),
                    }
                )
            ...
            lending_df.to_excel(
                writer, sheet_name="Lending Recommendations", index=False
            )

        return "financial_analysis_report.xlsx"

    def load_bank_statement(
        self, uploaded_file, customer_id="AUTO", customer_name="Unknown"
    ):
        ...
        try:
            ...
            self.ml_df = (
                customer  # Store the DataFrame for use in all analysis functions
            )
            return [customer]
        except Exception as e:
...
def rule_based_ai_analysis(customer_data):
    ...
    if income > 50000:
        if savings < 0.15 * income:
            analysis.append(
                "🔍 **Income Analysis**: High income but low savings - potential lifestyle inflation"
            )
        else:
            analysis.append(
                "✅ **Income Analysis**: Strong income with good savings discipline"
            )
    elif income > 25000:
        analysis.append(
            "📊 **Income Analysis**: Moderate income level - focus on expense optimization"
        )
    else:
        analysis.append(
            "⚠️ **Income Analysis**: Lower income - prioritize essential expenses"
        )

    # Savings Behavior Analysis
    if savings > 0.3 * income:
        analysis.append(
            "🏆 **Savings Excellence**: Outstanding savings rate - eligible for premium products"
        )
    elif savings > 0.2 * income:
        analysis.append(
            "✅ **Good Savings**: Above-average savings - consider investment products"
        )
    elif savings > 0.1 * income:
        analysis.append(
            "📈 **Moderate Savings**: Adequate savings - room for improvement"
        )
    else:
        analysis.append(
            "⚠️ **Low Savings**: Below recommended savings rate - financial education needed"
        )
    ...
    if savings > 0.2 * income:
        recommendations.append(
            "🏦 **Fixed Deposits**: High savings - suggest FD products"
        )
    if income > 30000 and savings < 0.15 * income:
        recommendations.append(
            "📱 **Recurring Deposits**: Help build savings discipline"
        )
    if income > 50000:
        recommendations.append(
            "🏠 **Home Loan**: High income - pre-approved home loan eligible"
        )
    ...
    if upload_option == "Upload CSV File":
        ...
        if uploaded_file is not None:
            ...
            if use_enhanced_model:
                ...
                print(
                    "Enhanced model predictions (before rules):",
                    pd.Series(model_labels).value_counts(),
                )
                ...
                if bert_embedder is None:
                    st.error(
                        "BERT embedder model is missing. Please upload 'bert_embedder.pkl' to the app directory."
                    )
                    return
                combined_text = (
                ...
                print(
                    "BERT model predictions (before rules):",
                    pd.Series(model_labels).value_counts(),
                )
                label_and_source = [
                    hybrid_label_rule(row, model_labels[i]) for i, row in df.iterrows()
                ]
            ...
            mask_credit_override = credit_mask & ~df["Predicted_Label"].isin(
                ["Savings", "Income"]
            )
            df.loc[mask_credit_override, "Predicted_Label"] = "Income"
            df.loc[mask_credit_override, "Label_Source"] = "Rule-Income-Force"
            mask_debit_override = debit_mask & ~df["Predicted_Label"].isin(
                ["Savings", "Expense"]
            )
            ...
            print(
                df[df["Type"].str.upper() == "CREDIT"][
                    ["Type", "Description", "Predicted_Label", "Label_Source"]
                ].head(10)
            )
            customers = df.to_dict("records")
        ...
        customer_name = st.sidebar.text_input(
            "Customer Name (optional)", value="Unknown"
        )
        customers = []
        failed_files = []
        ...
        if uploaded_files:
            for uploaded_file in uploaded_files:
                try:
                    ...
                    if "Type" in df.columns:
                        ...
                        print(
                            "Type values after DR/CR conversion:", df["Type"].unique()
                        )
                        print(
                            df[
                                [
                    ...
                    if not missing:
                        ...
                        if (
                            "date" in cols
                            and "amount" in cols
                            and ("remarks" in cols or "description" in cols)
                            and "type" in cols
                        ):
                            desc_col = (
                                "description" if "description" in cols else "remarks"
                            )
                            std_df = df.rename(
                                columns={
                        ...
                        # SBI style (all lowercase)
                            ...
                            df["Type"] = df.apply(
                                lambda row: (
                                    ...
                                    and str(row["debit"]).strip() != ""
                                    else (
                                        "CREDIT"
                                        if pd.notnull(row["credit"])
                                        and str(row["credit"]).strip() != ""
                                        else ""
                                    )
                                ),
                                axis=1,
                        ...
                        # Fallback: try to use any available date, amount, type, description columns
                        elif set(["date", "description", "amount", "type"]).issubset(
                            set(cols)
                        ):
                            std_df = df.rename(
                                columns={
                            ...
                            st.sidebar.error(
                                f"❌ Could not auto-detect format for file: {uploaded_file.name}. Columns found: {df.columns.tolist()}"
                            )
                            continue
                        # Standardize types
                        std_df["Description"] = std_df["Description"].astype(str)
                        std_df["Amount"] = pd.to_numeric(
                            std_df["Amount"], errors="coerce"
                        )
                        std_df["Type"] = (
                            std_df["Type"]
                            .astype(str)
                    ...
                    if std_df is not None and not std_df.empty:
                        ...
                        if "Amount" in std_df.columns:
                            ...
                            std_df = std_df[
                                std_df["Amount"].astype(str).str.strip() != ""
                            ]
                            ...
                            if std_df.empty:
                                st.sidebar.error(
                                    f"❌ All rows in {uploaded_file.name} had invalid or missing Amount values. Please check your file."
                                )
                                continue
                        # --- Check for required columns before ML prediction ---
                        required_cols = ["Date", "Description", "Amount", "Type"]
                        missing_cols = [
                            col for col in required_cols if col not in std_df.columns
                        ]
                        if std_df.empty or missing_cols:
                            st.sidebar.error(
                                f"❌ {uploaded_file.name} is missing required columns: {missing_cols} or is empty. Skipping ML prediction."
                            )
                            std_df["Predicted_Label"] = "Unknown"
                            customers.append(std_df)
                            continue
                        # Add Merchant_Category
                        std_df["Merchant_Category"] = std_df["Description"].apply(
                            extract_merchant_category
                        )

                        # Use enhanced model if available, otherwise fallback to BERT
                        if use_enhanced_model:
                            # Prepare text features for enhanced model
                            std_df["text_features"] = (
                                std_df["Description"].fillna("")
                                + " "
                                + std_df["Type"].fillna("")
                            )
                            X_vec = enhanced_vectorizer.transform(
                                std_df["text_features"]
                            )
                            model_labels = enhanced_model.predict(X_vec)
                            print(
                            ...
                            if bert_embedder is None:
                                ...
                                return
                            combined_text = (
                                std_df["Type"].astype(str).str.upper().str.strip()
                                + ": "
                                + std_df["Description"].astype(str).str.strip()
                                + " | "
                                + std_df["Merchant_Category"].astype(str).str.strip()
                            ).tolist()
                            X = bert_embedder.encode(
                            ...
                            label_and_source = [
                                hybrid_label_rule(row, model_labels[i])
                                for i, row in std_df.iterrows()
                            ]
                            std_df["Predicted_Label"] = [
                                ls[0] for ls in label_and_source
                            ]
                        ...
                        mask_credit_override = credit_mask & ~std_df[
                            "Predicted_Label"
                        ].isin(["Savings", "Income"])
                        std_df.loc[mask_credit_override, "Predicted_Label"] = "Income"
                        std_df.loc[mask_credit_override, "Label_Source"] = (
                            "Rule-Income-Force"
                        )
                        mask_debit_override = debit_mask & ~std_df[
                            "Predicted_Label"
                        ].isin(["Savings", "Expense"])
                        std_df.loc[mask_debit_override, "Predicted_Label"] = "Expense"
                        std_df.loc[mask_debit_override, "Label_Source"] = (
                            "Rule-Expense-Force"
                        )
                        print(
                            "Predicted_Label value counts:",
                            std_df["Predicted_Label"].value_counts(),
                        )
                        print("Sample CREDIT rows after override:")
                        print(
                            std_df[std_df["Type"].str.upper() == "CREDIT"][
                                [
                                    "Type",
                                    "Description",
                                    "Predicted_Label",
                        ...
                        st.sidebar.error(
                            f"❌ Parsed DataFrame is empty for file: {uploaded_file.name}"
                        )
                except Exception as e:
                    st.sidebar.error(
                        f"❌ Could not read file: {uploaded_file.name}. Error: {e}"
                    )
            ...
            if customers:
                ...
                dashboard.ml_df = (
                    all_df  # So display_customer_overview can use Predicted_Label
                )
        ...
        if os.path.exists(cleaned_csv_path):
            ...
            model_labels = bert_clf.predict(X)
            print(
                "Raw model predictions (before rules):",
                pd.Series(model_labels).value_counts(),
            )
            label_and_source = [
                hybrid_label_rule(row, model_labels[i]) for i, row in df.iterrows()
            ...
            customers = df.to_dict("records")
            st.sidebar.success(
                f"✅ Loaded {len(customers)} transactions from cleaned CSV"
            )
            dashboard.analyze_customers(customers)
        else:
            st.sidebar.error(
                f"❌ {cleaned_csv_path} not found. Please generate it first."
            )
    else:
